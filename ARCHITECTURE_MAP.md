...
- **UiTheme** (*static*, Батч 13) — текущий 9-slice спрайт панелей + `SetPanelSprite` (применяет ко всем `ThemedPanel`). Выбор от `CustomizationUI`, персист `themeId`.
...
- **GemShopUI** (*singleton*, Батч 13) — магазин кристаллов (код-билд): 4 товара → `GameManager.BuyGems`. Вход — тап по строке «Кристаллы» в `CurrencyHudUI`.
- **TimedOfferUI** (*singleton*, Батч 13) — таймовый оффер «Стартовый набор» с отсчётом (код-билд), 1×/сессию на дне 3 после рекламы.
- **CustomizationUI** (*singleton*, Батч 13) — экран оформления: аватар (`Mini UI/Avatars`) + тема (`Dark Theme RoundEdge`). Спрайты от билдера, сетки в коде. Персист `GameManager.AvatarId/ThemeId`. Применение: аватар→бейдж HUD (тап открывает, виден с D4), тема→`UiTheme`. На всегда-активном Canvas.
- **ThemedPanel** (*MonoBehaviour*, Батч 13) — маркер темизируемой панели; на `OnEnable`/по `UiTheme` меняет 9-slice спрайт. Билдер вешает в `ApplyPanelSprite`.
