...
- **Analytics** (*static*, Батч 10) — единая точка отправки событий в Яндекс Метрику (`YG2.MetricaSend`). Константы-имена = «Цели» в Метрике. Воронка оттока: `session_start`, `tutorial_start/done`, `day_start/complete/failed` (param day/mode), `ad_interstitial`, `ad_rewarded`(id), `purchase`(product), `journey_gate`, `endless_start`, `prompt_shown/accepted`(which). Модуль Metrica включается настройкой `infoYG.Metrica.enable`+CounterID (НЕ дефайном), поэтому вызовы без `#if`.
- **DailyTaskBoard** (*static*, Батч 15) + `DailyTask` — 3 задачи/день (пул `DailyChallenge.Kind`), клейм→монеты, бонус «все три»→+монеты/+кристаллы. Заменил одиночный «заказ дня». Сейв `dailyTasks*`.
- **RecipeBook** (*static*, Батч 15) — мастерство напитков: `ReportServe`/`Level`/`MasteryBonus(+3%/ур)`. Сейв `recipeKeys/Served`.
- **EventManager** (*static*, Батч 15) — недельный турнир (ISO-неделя), очки=звёзды, вехи→кристаллы. Сейв `event*`.
- **SeasonPass** (*static*, Батч 15) — 30 ур., free-монеты/premium-кристаллы (`BuyPremiumWithGems 150`). Сейв `pass*`.
- Отношения — в **GameManager** (`journalRelXp`, `GetRelationshipLevel`, `RelationshipTipBonus`). XP событий/сезона — со звёзд в `DayController`.
- **RenovationManager** (*static*, Батч 15) — обустройство кофейни (гл. сток монет): 12 проектов (цена/выгода `Price/Tip/Tolerance`/сюжет), `Complete()`/`CompleteWithGems(30)`, бонусы читают `GameManager.PriceMultiplier/MoodBonus/ToleranceBonus`. Сейв `renovationStage`.
- **RenovationVisualizer** (*MonoBehaviour*, Батч 15) — ставит мебель-префабы `LowPolyDungeon` по якорям `RenoAnchors` на завершённые стадии; `_stagePrefabs[12]`/`_anchors[12]` от билдера; `ShowStage`/опц. `ShowcaseStage`(камера).
- **RushController** (*static*, Батч 11) — «Час пик»: `IsRushDay(displayDay,endless)` (всегда в endless, в сюжете с 6-го дня при `day%3==0`), `SpeedMultiplier(elapsed)` (1…×1.30 за темп), `InTime`. Чистая логика темпа поверх последовательной подачи (ядро `ServeCustomer` НЕ переписано). Читает `DayController`; визуал — `RushHudUI`.
- **YaShortcut** (*static*, Батч 10) — кастомный мост к `ysdk.shortcut` (ярлык на рабочий стол): `Available()`/`Prompt()` через `Assets/Plugins/YaShortcut.jslib` (плагин YG2 такого API не даёт). Всё под `typeof/try-catch` → безопасный no-op, если ysdk недоступен. Только WebGL, требует проверки на реальной сборке.
...
- **DayResultUI** (344) — экран итогов дня: rewarded «Удвоить»/«Сохранить комбо», трекер «Путь к 10000», предупреждение о стрике. Батч 11: `Show(...,titleOverride,endlessBestDay)` — в endless плашка «БЕСКОНЕЧНЫЙ ДЕНЬ N» + трекер переиспользован под «Рекорд: бесконечный день N».
- **GemShopUI** (*singleton*, Батч 13/переработан Батч 14) — «Кристаллы»: секция **Потратить** (перки: мгновенный апгрейд кофейни за `GemUpgradeCost`, убрать рекламу за `GemRemoveAdsCost`) + **Пополнить** (IAP `BuyGems`). Вход — тап по строке «Кристаллы» в `CurrencyHudUI`. Код-билд.
- **DailyTaskBoardUI** / **ProgressHubUI** / **RewardWheelUI** (*singletons*, Батч 15) — код-билд: доска задач (лев. кнопка+бейдж), хаб прогресса (кнопка «Прогресс» + меню Рецепты/Альбом/Событие/Сезон/Колесо), колесо удачи (дневной спин + rewarded).
- **RenovationGoalUI** (*singleton*, Батч 15) — постоянный виджет цели обустройства (левая колонка под кассой/кристаллами) «Копим: X A/B» + панель проекта (сюжет/выгода/«Обустроить»/«Ускорить»). Гл. «грид денег». Код-билд.
- **CurrencyHudUI** (*singleton*, Батч 14) — HUD **только кристаллов** (монеты — `CoinsUI`), одна строка слева-сверху под кассой (не пересекает правый док); тап открывает `GemShopUI`. Иконка `_gemIcon` от билдера.
- **TimedOfferUI** (*singleton*, Батч 13/14) — таймовый оффер «Стартовый набор» с отсчётом (код-билд), 1×/сессию на **дне 5** после рекламы; не поверх активного `RewardPopupUI`.
- (удалено в Батч 14: `CustomizationUI`, `ThemedPanel`, `UiTheme` — кастомизация убрана.)
- **RushHudUI** (*singleton*, Батч 11) — HUD «Часа пик»: плашка «Час пик · очередь: N» + убывающая шкала темпа. Строится В КОДЕ (свой overlay-Canvas, как AdRemovalPrompt; `_font` от билдера, фолбэк `TMP_Settings.defaultFontAsset`). API: `BeginRush/StartTimer/StopTimer/SetQueue/EndRush`. Шкала по масштабируемому времени (пауза замораживает). Если билдер не создал — DayController деградирует молча (null-safe).
- **EngagementPrompt** (*singleton*, Батч 10) — переиспользуемый промпт «вопрос + Да/Нет», строится В КОДЕ (как AdRemovalPrompt, без билдера). `Ensure()`/`Show(titleRu,acceptRu,declineRu,onAccept)`; локализация «плашками» через `UiTranslations`. Триггерит `GameManager.MaybeShowEngagementPrompt`: день 1 → ярлык (`YaShortcut`), день 3 → оценка (`#if Review_yg YG2.ReviewShow`). Флаги `shortcutAsked/reviewAsked` в сейве.
- **TutorialController** (292) — обучение. **UiEffects** (263, *singleton*) — 2D эффекты (монеты/звёзды/комбо/баннеры). **HintManager** (166) — панель подсказок. **UpgradeShopUI** (121) — магазин апгрейдов; Батч 11: шкалы «сейчас/станет» (`_effectFills/_effectGhosts`). **SettingsUI** (128) — настройки+пауза (громкость/фуллскрин/лидерборд); Батч 11: 2-я кнопка+панель «Рекорды: бесконечный режим» (таблица `endless`, видна только при `EndlessActive`). **DailyBonusUI** (123) — бонус за вход. **JourneyGateUI** (102) — гейт цели. **GuestJournalUI** (73) + **JournalCard** (32) — журнал «Завсегдатаи». **AdForCoins** (44) — реклама за монеты. **CoinsUI** (30) — касса, `Update()`.

### Audio / SDK / CutScene / Effects / прочее
- **AudioController** (*singleton*) — музыка/SFX/бубнёж. **Три независимых канала громкости** (Батч 9): `MusicVolume`, `SfxVolume`, `VoiceVolume` (бубнёж; `SpeechMixer` читает его, а не Sfx). Сейв `music/sfx/voiceVolume` (дефолты 0.5/0.8/0.9); `SetVolumes(m,s,v)`/`ApplySavedVolumes(m,s,v)` — 3 аргумента. ⚠️ SFX дополнительно приглушены константой `SfxGain=0.35` в `PlaySFX` (эффекты по задумке заметно тише музыки/голосов даже при ползунке на максимуме). Превью: `PlaySfxPreview` (реальный SFX-клик), `PlayVoicePreview` (бубнёж). Клипы берёт из **`SoundBank`** (`_bank`, приоритет) с фолбэком на старые поля. Методы `PlayCoin/Star/...`, `PlayClick/UiOpen/UiClose/Combo`, `PauseMusic/ResumeMusic` (музыка молчит ночью), **`PlayNight/StopNight`** — ночной эмбиент `_nightAmbience` (`Assets/Audio/night_ambience.mp3`, отдельный зацикленный `_nightSource`; вкл. во «Сне», выкл. до рекламы). ⚠️ Порядок между днями: сон → StopNight → реклама (`Transition`) → ResumeMusic.
